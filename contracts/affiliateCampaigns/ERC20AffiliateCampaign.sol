...
contract ERC20AffiliateCampaign is CampaignAidropsShared {
    ...
    uint256 private costPerParticipant;
    ...
    address internal campaignFunder;
    ...
    function fundCampaign() external {
        if (campaignFunded) revert AlreadyFunded();

        campaignFunded = true;
        campaignFunder = msg.sender;

        rewardToken.safeTransferFrom(msg.sender, address(this), tokenSupply);
        emit CampaignFunded(address(this));
    }
    ...
    function withdrawCampaignFunds() external {
        if (campaignFunder != msg.sender) revert Unauthorized();
    ...
    // influencerClaim()
    // participantClaim()
    // getRewardAmount() - for influencers & participants
