...
export function shouldChangeAdminInAllAirDrops(): void {
  ...
  it("change admin inheritance - ExistingTokenDrop", async function() {
    ...
    expect( await this.airbroFactory.connect(this.signers.deployer).dropExistingTokensToNftHolders(
        this.testNftCollection.address, // rewardedNftCollection,
        100, // tokensPerClaim
  ...
  it("change admin inheritance - Existing1155NftDrop", async function() {
    ...
    expect( await this.airbroFactory.connect(this.signers.deployer).dropExisting1155NftsToNftHolders(
    ...
    const existing1155NftDropContract = existing1155NftDropFactory.attach(await this.airbroFactory.airdrops(0));
