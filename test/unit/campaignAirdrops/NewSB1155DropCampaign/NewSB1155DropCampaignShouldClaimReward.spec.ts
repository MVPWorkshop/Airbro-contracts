...
export function NewSB1155DropCampaignShouldClaimReward(): void {
  describe("user should be able to claim reward", async function () {
    it("should be able to claim if part of merkleRoot", async function () {
      ...
      void expect(
        await this.newSB1155DropCampaign
          .connect(this.signers.alice)
          .claim(hexProof, this.signers.alice.address, { value: claimFee }),
      )
      ...
      expect(await this.newSB1155DropCampaign.balanceOf(this.signers.alice.address, constants.Zero)).to.be.equal(
        constants.One,
      );
    });
    ...
    it("should revert claim if not part of merkleRoot", async function () {
      ...
      await expect(
        this.newSB1155DropCampaign
          .connect(this.signers.peter)
          .claim(hexProof, this.signers.peter.address, { value: claimFee }),
    ...
    it("function isEligibleForReward() should return true if eligible for reward", async function () {
      ...
      const isEligibleForReward = await this.newSB1155DropCampaign
        .connect(this.signers.alice)
    ...
    it("function isEligibleForReward() should return false if user already redeemed reward", async function () {
      ...
      expect(await this.newSB1155DropCampaign.connect(this.signers.alice).isEligibleForReward(hexProof)).to.be.equal(
        false,
      );
    });
