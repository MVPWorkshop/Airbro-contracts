...
export function NewERC1155DropCampaignShouldClaimReward(): void {
  describe("user should be able to claim reward", async function () {
    it("should be able to claim if part of merkleRoot", async function () {
      ...
      expect(await this.newERC1155DropCampaign.connect(this.signers.alice).claim(hexProof, this.signers.alice.address, { value: claimFee }))
    ...
    it("should revert claim if not part of merkleRoot", async function () {
      ...
      await expect(
        this.newERC1155DropCampaign.connect(this.signers.peter).claim(hexProof, this.signers.peter.address, { value: claimFee }),
    ...
    it("should revert claim if already redeemed", async function () {
      ...
      await expect(
        this.newERC1155DropCampaign.connect(this.signers.alice).claim(hexProof, this.signers.alice.address, { value: claimFee }),
    ...
    it("should return correct amount from isEligibleForReward method", async function () {
      ...
      // alice claiming her reward
      await this.newERC1155DropCampaign.connect(this.signers.alice).claim(hexProof, this.signers.alice.address, { value: claimFee });
