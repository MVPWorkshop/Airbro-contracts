...
export const AirbroFactory1155Holder = (): void => {
    ...
    describe('should deploy new 1155 NFT Collection',async function(){
        ...
        .withArgs(await this.airbroFactory1155Holder.nft1155Contracts(constants.Zero), this.signers.deployer.address);
        })

        it('should allow user to mint an NFT from the new collection',async function(){
        await this.airbroFactory1155Holder.createNewNft1155Contract("ipfs://bafybeict2kq6gt4ikgulypt7h7nwj4hmfi2kevrqvnx2osibfulyy5x3hu/no-time-to-explain.jpeg")
        ...
        expect( await collection1155.connect(this.signers.alice).balanceOf(this.signers.alice.address, constants.Zero)).to.be.equal(constants.One);

        })

        it('should increment totalNft1155ContractsCount for each 1155 NFT Collection created',async function(){
            await this.airbroFactory1155Holder.createNewNft1155Contract("ipfs://bafybeict2kq6gt4ikgulypt7h7nwj4hmfi2kevrqvnx2osibfulyy5x3hu/no-time-to-explain.jpeg")
            expect(this.airbroFactory1155Holder.totalNft1155ContractsCount).to.be.equal(constants.One);

            await this.airbroFactory1155Holder.createNewNft1155Contract("ipfs://bafybeict2kq6gt4ikgulypt7h7nwj4hmfi2kevrqvnx2osibfulyy5x3hu/no-time-to-explain.jpeg")
            expect(this.airbroFactory1155Holder.totalNft1155ContractsCount).to.be.equal(constants.Two);
        })
