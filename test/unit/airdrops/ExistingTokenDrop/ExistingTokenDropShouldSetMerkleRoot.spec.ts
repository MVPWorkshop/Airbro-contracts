...
export const ExistingTokenDropShouldSetMerkleRoot = (): void => {
  ...
  describe("should set merkleRoot", async function () {
    it("should allow admin to set merkleRoot", async function () {
      await expect(this.existingTokenDrop.connect(this.signers.backendWallet).setMerkleRoot(bytes32MerkleRootHash))
        .to.emit(this.existingTokenDrop, "MerkleRootChanged")
    ...
    it("should revert merkleRoot change from non admin account", async function () {
      ...
      await expect(this.existingTokenDrop.connect(nonOwnerAccount).setMerkleRoot(bytes32MerkleRootHash)).to.be.revertedWith("Unauthorized");
    });
  });
};
