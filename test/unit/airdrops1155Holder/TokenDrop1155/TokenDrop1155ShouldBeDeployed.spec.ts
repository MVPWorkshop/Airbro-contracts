...
export const TokenDrop1155ShouldDeploy = (): void => {
    ...
    describe('should deploy',async function(){
        ...
        it('should have rewardedNft set to correct hardcoded address',async function(){
            expect(await this.tokenDrop1155.rewardedNft()).to.be.equal(this.constructorArgs.rewardedNft)
        })
        
        it('should have tokensPerClaim set to correct amount',async function(){
            expect(await this.tokenDrop1155.tokensPerClaim()).to.be.equal(this.constructorArgs.tokensPerClaim)
        })
        
        it('should have token name set to correct name',async function(){
            expect(await this.tokenDrop1155.name()).to.be.equal(this.constructorArgs.name)
        })
        
        it('should have token symbol set to correct symbol',async function(){
            expect(await this.tokenDrop1155.symbol()).to.be.equal(this.constructorArgs.symbol)
        })
        ...
        it('should have airdropDuration set to 1 day (specifically, 86400 seconds)',async function(){
            expect(await this.tokenDrop1155.airdropDuration()).to.be.equal(this.constructorArgs.airdropDuration * dayInSeconds)
        })
        ...
        it('expect airdropFinishTime to be correctly set',async function(){
            ...
            expect(await this.tokenDrop1155.airdropFinishTime()).to.be.equal(blockBefore.timestamp + this.constructorArgs.airdropDuration * dayInSeconds)
        })
