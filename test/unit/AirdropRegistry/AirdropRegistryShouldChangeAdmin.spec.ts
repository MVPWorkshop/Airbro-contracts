...
export function AirdropRegistryShouldChangeAdmin(): void {
  describe("airdrop registry admin changing functionality", async function () {
    it("should be able to change registry admin", async function () {
      ...
      await expect(this.airdropRegistry.connect(this.signers.registryAdmin).initiateAdminTranfer(this.signers.jerry.address))
        .to.emit(this.airdropRegistry, `AdminTransferInitiated`)
        .withArgs(this.signers.jerry.address);

      await expect(this.airdropRegistry.connect(this.signers.jerry).acceptAdminTransfer())
